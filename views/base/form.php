<div class="row">
	<div class="col-lg-12">
		<button type="button" class="btn painel-option" data-option="bootstrap">Bootstrap</button>
		<button type="button" class="btn painel-option" data-option="calculadora">Calculadora</button>
		<button type="button" class="btn painel-option" data-option="cores">Cores</button>
		<button type="button" class="btn painel-option" data-option="busca_painel">Busca</button>
		<button type="button" class="btn item-painel" data-option="item_teste">Item</button>
		<button type="button" class="btn item-painel" data-option="gerador_docs">Gerar Docs</button>
	</div>
</div>
<div class="row">
	<div class="col-lg-12" id="base-content"></div>
	<div class="col-lg-12" id="item-list"></div>
</div>




