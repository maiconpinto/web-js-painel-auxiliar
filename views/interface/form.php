<div class="row">
	<div class="col-lg-12">
		<button type="button" class="btn get-module" data-option="bootstrap_help">Bootstrap</button>
		<button type="button" class="btn get-module" data-option="calculadora">Calculadora</button>
		<button type="button" class="btn get-module" data-option="cores">Cores</button>
		<button type="button" class="btn get-module" data-option="search">Busca</button>
		<button type="button" class="btn get-module" data-option="gerador_docs">Gerar Docs</button>
	</div>
</div>
<div class="row">
	<div class="col-lg-12" id="module-list"></div>
</div>